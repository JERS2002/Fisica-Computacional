...
def equipartition_gaussian(x, stdequi):
    return 0.6/ (stdequi * np.sqrt(2 * np.pi)) * np.exp(-x**2 / (2 * stdequi**2))
...
plt.xlabel('$v_y$')
plt.ylabel('P($v_y$)')
plt.title('Distribución de la componente y de la velocidad')
plt.legend()

# Guardar la gráfica como imagen PNG
output_filename = "histogramavy4.png"
